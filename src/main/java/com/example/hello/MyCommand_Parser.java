...
final class MyCommand_Parser {
  ...
  void printOnlineHelp(PrintStream printStream) {
    ...
    printOption(printStream, Option.VERBOSITY);
  }

  private void printOption(PrintStream printStream, Option option) {
    String shape_padded_24_characters = String.format("  %1$-22s", option.shape);
    ...
    printTokens(printStream, 25, tokens);
  }
  ...
  private static class StatefulParser {
    ...
    MyCommand build() {
      return new MyCommandImpl(
          paramParsers[0].stream().map(Paths::get).findAny().orElseThrow(Option.PATH::missingRequired),
  ...
  private enum Option {
    ...
    VERBOSITY(Collections.singletonList("--verbosity"), null,
        Collections.singletonList("An {@code @Option} is a named option."),
        "--verbosity VERBOSITY");
