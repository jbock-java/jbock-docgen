...
final class MyArguments_Parser {
  ...
  private static class Tokenizer {
    ...
    ParseResult parse(String[] args) {
      try {
        Optional<? extends MyArguments> result = parse(Arrays.asList(args).iterator());
        if (result.isPresent()) {
        ...
        err.decrementIndent();
        err.println();
        err.println(String.format("Try '%s --help' for more information.", "MyArguments"));
        err.println();
    ...
    Optional<? extends MyArguments> parse(Iterator<String> tokens) {
      boolean first = true;
  ...
  private static class Helper {
    ...
    Optional<? extends MyArgumentsImpl> build(List<String> positional) {
      int max = 1;
      int size = positional.size();
      if (size > max) {
        throw new IllegalArgumentException("Invalid option: " + positional.get(max));
      }
      return Optional.of(new MyArgumentsImpl(
          Option.PATH.value(positional).map(Paths::get).orElseThrow(Option.PATH.missingRequired()),
