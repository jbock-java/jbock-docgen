...
final class JbockAutoTypes_Parser {
  ...
  ParseResult parse(String[] args) {
    ...
    try {
      JbockAutoTypes result = state.parse(it).build();
  ...
  void printOnlineHelp() {
    ...
    Collections.addAll(description, "that can be used without a custom converter in jbock 4.3.001:</p>".split("\\s+", -1));
  ...
  private static class StatefulParser {
    ...
    StatefulParser() {
      ...
      optionNames.put("--bigdecimal", Option.BIG_DECIMAL);
      optionParsers.put(Option.BIG_DECIMAL, new RegularOptionParser(Option.BIG_DECIMAL));
      optionNames.put("--biginteger", Option.BIG_INTEGER);
      optionParsers.put(Option.BIG_INTEGER, new RegularOptionParser(Option.BIG_INTEGER));
      optionNames.put("--uri", Option.U_RI);
      optionParsers.put(Option.U_RI, new RegularOptionParser(Option.U_RI));
      optionNames.put("--path", Option.PATH);
      optionParsers.put(Option.PATH, new RegularOptionParser(Option.PATH));
      optionNames.put("--localdate", Option.LOCAL_DATE);
      optionParsers.put(Option.LOCAL_DATE, new RegularOptionParser(Option.LOCAL_DATE));
